...
-(void)addAll {
    if ((self.xl_buttonNames.count || self.xl_views.count) && self.xl_buttonNames.count !=self.xl_views.count) {
        ...
        self.xl_buttonNames =self.xl_buttonNames?self.xl_buttonNames:@[@"111",@"222",@"333"];
...
-(void)addScroll1{
    ...
    if (self.xl_showLine) {
        ...
        lineLabel.frame = CGRectMake(0, height(self.scroll1.frame)-0.2, width(self.frame), 0.2);
        lineLabel.backgroundColor = [UIColor blackColor];
        [self.scroll1 addSubview:lineLabel];
    }
    ...
    self.view2 =[[UIView alloc]initWithFrame:CGRectMake(_x0,CGRectGetMaxY(button0.frame)-(self.xl_buttonToSlider?self.xl_buttonToSlider:10), size0.width, self.xl_sliderHeight)];
...
-(void)addScroll2{
    self.scroll2 =[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, screen_width, self.frame.size.height)];
...
-(void)changed:(UIButton *)button{
    ...
    self.index = button.tag-5000;
    NSLog(@"%d",self.index);
    if (_indexDelegate && [_indexDelegate respondsToSelector:@selector(XLGaveIndex:index:)]) {
        [_indexDelegate XLGaveIndex:self index:self.index];
    }
